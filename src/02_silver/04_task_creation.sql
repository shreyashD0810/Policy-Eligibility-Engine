...
CREATE OR REPLACE TASK process_customer_master_task
    WAREHOUSE = policy_engine_silver_wh
    SCHEDULE = '1 MINUTE'  -- Checks every minute
    WHEN SYSTEM$STREAM_HAS_DATA('customer_master_stream') -- Only runs if new data exists
AS
...
CREATE OR REPLACE TASK process_claims_history_task
    WAREHOUSE = policy_engine_silver_wh
    SCHEDULE = '1 MINUTE'  -- Checks every minute
    WHEN SYSTEM$STREAM_HAS_DATA('CLAIMS_HISTORY_STREAM') 
AS
...
CREATE OR REPLACE TASK process_region_mapping_task
    WAREHOUSE = policy_engine_silver_wh
    SCHEDULE = '1 MINUTE'  -- Checks every minute
    WHEN SYSTEM$STREAM_HAS_DATA('REGION_MAPPING_STREAM') 
AS
...
CREATE OR REPLACE TASK process_product_family_rules_task
    WAREHOUSE = policy_engine_silver_wh
    SCHEDULE = '1 MINUTE'  -- Checks every minute
    WHEN SYSTEM$STREAM_HAS_DATA('PRODUCT_FAMILY_RULES_STREAM') 
AS
...
CREATE OR REPLACE TASK process_kyc_records_task
    WAREHOUSE = policy_engine_silver_wh
    SCHEDULE = '1 MINUTE'  -- Checks every minute
    WHEN SYSTEM$STREAM_HAS_DATA('KYC_RECORDS_STREAM') 
AS
...
CREATE OR REPLACE TASK process_watchlist_task
    WAREHOUSE = policy_engine_silver_wh
    SCHEDULE = '1 MINUTE'  -- Checks every minute
    WHEN SYSTEM$STREAM_HAS_DATA('WATCHLIST_STREAM') 
AS
...
ALTER TASK process_claims_history_task RESUME;
ALTER TASK process_customer_master_task RESUME;
ALTER TASK process_kyc_records_task RESUME;
ALTER TASK process_product_family_rules_task RESUME;
ALTER TASK process_region_mapping_task RESUME;
ALTER TASK process_watchlist_task RESUME;